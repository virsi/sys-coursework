...
;  tlink /t /x tsr.obj
;
;  МГТУ им. Н.Э. Баумана, ИУ5-42Б, 2026 г.
;  Вариант №5: F8 - подпись (5 сек, верх), F9 - курсив 'З',
;              F1 - русификация ЕЖЗИК, F2 - ограничить русские буквы.
;  Воробьев Е. А.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
code segment	'code'
	...
	; данные
	;replaceWith 				DB	'' 
	;@ Вариант №5: ограничить ввод только русских букв (строчные + прописные)
	ignoredChars                DB  'абвгдеёжзийклмнопрстуфхцчшщъыьэюяАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ'  ; русские буквы (cp866)
	ignoredLength 				equ	$-ignoredChars ; длина строки ignoredChars
	ignoreEnabled 				DB	0	; флаг функции игнорирования ввода
	;@ Вариант №5: русификация набора ЕЖЗИК (на ЙЦУКЕН раскладке: T -> Е, ; -> Ж, P -> З, B -> И, R -> К)
	translateFrom 				DB	'T;PBR'	; латинские клавиши на местах ЕЖЗИК
	translateTo 					DB	'ЕЖЗИК'	; русские буквы (cp866)
	translateLength				equ	$-translateTo	; длина строки translateFrom
...
;@ Вариант №5: курсивная буква 'З' 8x16 (с наклоном вправо)
cursiveSymbol           DB 00000000b ; матрица курсивной буквы 'З' (вместо обычного начертания)
						DB 00000000b
						DB 00111100b ; верхняя дуга
						DB 01100110b
						DB 01000110b
						DB 00000110b
						DB 00001100b ; изгиб вправо к середине
						DB 00111100b ; средняя горизонталь
						DB 00001100b
						DB 00011000b
						DB 00110000b
						DB 01100110b
						DB 01100110b
						DB 00111100b ; нижняя дуга
						DB 00000000b
						DB 00000000b

	charToCursiveIndex 		DB 'З' ; символ для замены (cp866)
	...
	;@ Вариант №5: ФИО, группа, номер варианта (длина первой строки задаёт ширину рамки)
	signatureLine1				DB	179, 'Воробьев Е. А.                                         ', 179
	...
	helpMsg DB '>tsr.com [/?] ', 10, 13
					...
					DB '  TSR.COM - повторный запуск выгружает резидент из памяти', 10, 13
					DB '  F8  - вывод ФИО и группы по таймеру (5 сек) в верх экрана', 10, 13
					DB '  F9  - включение и отключение курсивного вывода русского символа З', 10, 13
					DB '  F1  - включение и отключение частичной русификации клавиатуры (T, ;, P, B, R -> ЕЖЗИК)', 10, 13
					DB '  F2  - включение и отключение режима ограничения ввода русских букв', 10, 13
	...
	;@ Вариант №5: индикатор активных функций в правом нижнем углу
	f1_txt						DB	'F8' ; вывод подписи
	f2_txt						DB	'F9' ; курсив 'З'
	f3_txt						DB	'F1' ; русификация ЕЖЗИК
	f4_txt						DB	'F2' ; ограничение русских букв
    ...
    new_int9h proc far
		...
		;@ Вариант №5: проверка функциональных клавиш F8, F9, F1, F2
		;@ Скан-коды: F1=3Bh F2=3Ch F8=42h F9=43h
		;@ После sub AL,58: F1->1 F2->2 F8->8 F9->9

		;проверка F8/F9/F1/F2
		_test_Fx:
		sub AL, 58 ; в AL теперь номер функциональной клавиши (F1=1..F10=10)
		_F8:
			cmp AL, 8 ; F8 - подпись
			jne _F9
		...
		_F9:
			...
			jmp _translate_or_ignore
		_F1:
			cmp AL, 1 ; F1 - русификация ЕЖЗИК
			jne _F2
			not translateEnabled
			call changeFx
			jmp _translate_or_ignore
		_F2:
			cmp AL, 2 ; F2 - ограничение ввода русских букв
